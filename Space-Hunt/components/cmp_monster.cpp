...
MonsterComponent::MonsterComponent(Entity* p, shared_ptr<Entity> player) : ActorMovementComponent(p),
                                                                           _health(100), _player(std::move(player)),
                                                                           _attackTime(0), _damage(2), _deathTime(0.f) {
}
...
int MonsterComponent::get_health() const
{
	return _health;
}

void MonsterComponent::set_damage(const int damage)
{
	_damage = damage;
}

void MonsterComponent::attack(float dt)
{
	_attackTime -= dt;
...
void MonsterComponent::update(const float dt)
{
